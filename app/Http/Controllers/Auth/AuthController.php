...
class AuthController extends Controller
{
    ...
    */
    ...
    public function handleProviderCallback()
    {
        ...
        $password = $this->hashPassword(substr(str_shuffle(str_repeat($x='0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ', ceil(6/strlen($x)) )),1,6), $salt);
			
		//$pwd=generateRandomString();
		   DB::table('users')->insert(
    ['username' =>$username , 'email' =>$user->getEmail(),'password'=>$password, 'is_banned' => 0,'avatar_url'=> $user->getAvatar(),'score'=>20,'somaa'=>5,'facebook_id'=> $user->getId()]
);	   
		// event(new UserWasAddedEvent($user));
	...
	public function handleTwitterCallback()
    {
		...
		//echo "email "+$user->getEmail();
		 $found = User::where('twitter_id', $user->getId())->first();
        ...
        $password = $this->hashPassword(substr(str_shuffle(str_repeat($x='0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ', ceil(6/strlen($x)) )),1,6), $salt);
		   ...
		   DB::table('users')->insert(
    ['username' =>$username , 'email' =>$user->getId().'@twitter','password'=>$password, 'is_banned' => 0,'avatar_url'=> $user->getAvatar(),'score'=>20,'somaa'=>5,'twitter_id'=> $user->getId()]
);	   	
